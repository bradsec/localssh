...
func TestTerminalSizeValidation(t *testing.T) {
	tests := []struct {
		...
		{name: "minimum", cols: 1, rows: 1},
		{name: "maximum", cols: maxTerminalDimension, rows: maxTerminalDimension},
		{name: "zero columns", cols: 0, rows: 24, wantError: true},
		{name: "negative rows", cols: 80, rows: -1, wantError: true},
		{name: "columns too large", cols: maxTerminalDimension + 1, rows: 24, wantError: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateTerminalSize(tt.cols, tt.rows)
			if (err != nil) != tt.wantError {
				t.Fatalf("validateTerminalSize(%d, %d) error = %v", tt.cols, tt.rows, err)
			}
		})
	}
}
